...
namespace UnitTest
{
    ...
    public class SolveStateDictionaryTests
    {
        ...
        /// </summary>
        private SolveState CreateSolveState(byte[,] board, int gCost = 0, int hCost = 0, byte emptyRow = 0, byte emptyCol = 0)
        {
            return new SolveState(board, gCost, hCost, emptyRow, emptyCol);
        }
        ...
        public void AddState_MultipleStatesWithSameHash_CreatesCollision()
        {
            ...
            SolveState state1 = CreateSolveState(board1, emptyRow: 2, emptyCol: 2);
            SolveState state2 = CreateSolveState(board2, emptyRow: 2, emptyCol: 1);
        ...
        public void AddState_TrackMaxLength_WhenCollisionsIncrease()
        {
            ...
            SolveState state1 = CreateSolveState(board1, emptyRow: 2, emptyCol: 2);
            SolveState state2 = CreateSolveState(board2, emptyRow: 2, emptyCol: 1);
            SolveState state3 = CreateSolveState(board3, emptyRow: 1, emptyCol: 2);
            long hash = 12345L;
        ...
        public void Exists_StateExists_ReturnsTrue()
        {
            ...
            var dictionary = new SolveStateDictionary();
            byte[,] board = Create3x3Board(new byte[] { 1, 2, 3, 4, 5, 6, 7, 8, 0 });
            SolveState state = CreateSolveState(board, emptyRow: 2, emptyCol: 2);
            long hash = 12345L;
            dictionary.AddState(hash, state);
        ...
        public void TryGetState_StateExists_ReturnsStateAndTrue()
        {
            ...
            var dictionary = new SolveStateDictionary();
            byte[,] board = Create3x3Board(new byte[] { 1, 2, 3, 4, 5, 6, 7, 8, 0 });
            SolveState state = CreateSolveState(board, gCost: 5, hCost: 10, emptyRow: 2, emptyCol: 2);
            long hash = 12345L;
            dictionary.AddState(hash, state);
        ...
        public void TryGetState_MultipleStatesWithSameHash_ReturnsCorrectState()
        {
            ...
            byte[,] board1 = Create3x3Board(new byte[] { 1, 2, 3, 4, 5, 6, 7, 8, 0 });
            byte[,] board2 = Create3x3Board(new byte[] { 1, 2, 3, 4, 5, 6, 0, 8, 7 });
            SolveState state1 = CreateSolveState(board1, gCost: 5, emptyRow: 2, emptyCol: 2);
            SolveState state2 = CreateSolveState(board2, gCost: 6, emptyRow: 2, emptyCol: 1);
            long hash = 12345L;
            dictionary.AddState(hash, state1);
            dictionary.AddState(hash, state2);
